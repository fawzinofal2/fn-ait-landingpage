...
export async function generateMetadata({
  ...
  if (!app) return {};
  return { title: `${dict.appDetail.termsOfUse} — ${app.name}` };
}
...
export default async function AppTermsPage({
  ...
  return (
    <div>
      <PolicyDocument
        title={`${dict.appDetail.termsOfUse} — ${app.name}`}
        subtitle={dict.policyPages.appTermsSubtitle}
        sections={buildAppTermsOfUse(app, locale)}
      />
